...
export const registerReminderHandlers = (): void => {
  ipcMain.handle(REMINDER_CHANNELS.CREATE_REMINDERS_FOR_APPOINTMENT, async (_, appointmentId: number) => {
    return reminderService.createRemindersForAppointment(appointmentId);
  });

  ipcMain.handle(REMINDER_CHANNELS.GET_REMINDERS_BY_APPOINTMENT, async (_, appointmentId: number) => {
    return reminderService.getRemindersByAppointment(appointmentId);
  });

  ipcMain.handle(REMINDER_CHANNELS.CANCEL_REMINDERS, async (_, appointmentId: number) => {
